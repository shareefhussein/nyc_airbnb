...
def model_predictions():
    ...
    if dataset_pth is None: 
        dataset_pth = 'testdata.csv'
    
    df = pd.read_csv(os.path.join(test_data_path,dataset_pth))

    df_x, df_y, _ = data_preparation(df, encoder)
...
def execution_time():
    ...
    for code in ['training.py', 'ingestion.py']:
        starttime = timeit.default_timer()
        os.system('python3 %s' %code)
        timing=timeit.default_timer() - starttime
        res.append([code,timing])
